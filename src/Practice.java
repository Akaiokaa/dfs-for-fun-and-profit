...
public class Practice {
  ...
  private int max(Vertex<Integer> vertex, Set<Vertex<Integer>> visited) {
    ...
    for(Vertex<Integer> neighbor: vertex.neighbors){
      int maxOther = max(neighbor, visited);
      if (maxOther > max) {
        max = maxOther;
      }
    }
  ...
  public <T> Set<Vertex<T>> leaves(Vertex<T> vertex) {
    ...
    return leaves(vertex, new HashSet<>(), leavesSet);
  }

  private <T> Set<Vertex<T>> leaves(Vertex<T> vertex, Set<Vertex<T>> visited, Set<Vertex<T>> leavesSet) {
    if(vertex == null || visited.contains(vertex)) return new HashSet<>();
  ...
  public boolean allOdd(Vertex<Integer> vertex) {
    boolean bool = true;
    return allOdd(vertex, new HashSet<>(), bool);
  }
  // 1 - 3 - 5 - 7 - 9 - 2
  private boolean allOdd(Vertex<Integer> vertex, Set<Vertex<Integer>> visited, boolean bool) {
   if(vertex == null || visited.contains(vertex)) return true;
    ...
    for(Vertex<Integer> neighbor: vertex.neighbors){
      bool = allOdd(neighbor, visited, bool);
    }
    return bool;
